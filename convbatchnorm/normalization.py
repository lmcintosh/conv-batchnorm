...
__all__ = ['ConvBatchNormalization', 'ConvAdaptiveNormalization']
...
class ConvAdaptiveNormalization(Layer):
    """Convolutional adaptive normalization layer.

    Modification of batch normalization (Ioffe and Szegedy, 2014) that
    performs local spatial normalization while learning a single scale
    parameter across space for each channel.

    Unlike batch normalization, ConvAdaptiveNormalization has the
    same behavior during train and test time. In particular, the
    mean and variance is computed independently for each batch,
    rather than as a fixed parameter estimated as a moving average
    of statistics on the train set.
    ...
    def get_config(self):
        ...
        base_config = super(ConvAdaptiveNormalization, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
